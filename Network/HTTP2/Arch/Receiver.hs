  ...
  where
    ...
    loop n
      | n == 6 = do
        ...
        loop 0
      | otherwise = do
        hPutStrLn stderr "frameReceiver reading header"
        hd <- recvN frameHeaderLength
        hPutStrLn stderr "frameReceiver reading header - done"
        if BS.null hd
...
processFrame Context {..} recvN (FrameUnknown _, FrameHeader {payloadLength}) = do
  ...
  case mx of
    Nothing -> do
      ...
      hPutStrLn stderr "processFrame FrameUnknown reading"
      void $ recvN payloadLength
      hPutStrLn stderr "processFrame FrameUnknown done"
      return True
...
processFrame ctx recvN (FramePushPromise, header@FrameHeader {payloadLength})
  ...
  | otherwise = do
    hPutStrLn stderr "processFrame FramePushPromise reading"
    pl <- recvN payloadLength
    hPutStrLn stderr "processFrame FramePushPromise done"
  ...
  case checkFrameHeader settings typhdr of
    Left h2err -> case h2err of
      StreamError err sid -> do
        resetStream err sid
        hPutStrLn stderr "processFrame ftyp StreamError reading"
        void $ recvN payloadLength
        hPutStrLn stderr "processFrame ftyp StreamError done"
        return True
      connErr -> E.throwIO connErr
    Right _ -> do
      hPutStrLn stderr "processFrame ftyp Right reading"
      ex <- E.try $ controlOrStream ctx recvN ftyp header
      hPutStrLn stderr "processFrame ftyp Right done"
      case ex of
...
processState (Open (NoBody tbl@(_, reqvt))) ctx@Context {..} strm@Stream {streamInput} streamId = do
  ...
  if isServer ctx
    then atomically $ writeTQueue (inputQ roleInfo) $ Input strm inpObj
    ...
    if streamId <= csid -- consider the stream closed
      then
        if ftyp `elem` [FrameWindowUpdate, FrameRSTStream, FramePriority]
          ...
          else E.throwIO $ ConnectionError ProtocolError "stream identifier must not decrease"
      else do
...
control FrameSettings header@FrameHeader {flags} bs Context {http2settings, controlQ, firstSettings, streamTable, settingsRate} = do
  ...
  if testAck flags
    then return True
      ...
      if rate > settingsRateLimit
        then E.throwIO $ ConnectionError ProtocolError "too many settings"
        else do
  ...
  if testAck flags
    then return True
      ...
      if rate > pingRateLimit
        then E.throwIO $ ConnectionError ProtocolError "too many ping"
        else do
  ...
  if frag == "" && not endOfStream && not endOfHeader
    then do
      ...
      if rate > emptyFrameRateLimit
        then E.throwIO $ ConnectionError ProtocolError "too many empty headers"
        else return s
    else do
      case mp of
        Nothing -> return ()
      ...
      if endOfHeader
        then do
          ...
          return $
            if endOfStream
              then Open (NoBody tbl)
              else Open (HasBody tbl)
        else do
...
stream
  ...
  s@(Open (Body q mcl bodyLength _))
  _ = do
...
stream FrameContinuation FrameHeader {flags} frag ctx s@(Open (Continued rfrags siz n endOfStream)) _ = do
  ...
  if frag == "" && not endOfHeader
    then do
      ...
      rate <- getRate $ emptyFrameRate ctx
      if rate > emptyFrameRateLimit
        then E.throwIO $ ConnectionError ProtocolError "too many empty continuation"
        else return s
    else do
      ...
      if endOfHeader
        ...
        else return $ Open $ Continued rfrags' siz' n' endOfStream
